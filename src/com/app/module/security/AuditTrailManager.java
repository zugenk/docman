		...
		if(data!=null && !data.isEmpty()) {
			...
			if (filterMap!=null && !filterMap.isEmpty()) {
				...
				for (Iterator iterator = filterMap.keySet().iterator(); iterator.hasNext();) {
					...
					filterBuff.append(constructQuery("auditTrail",key,filterMap.get(key))); //filterBuff.append(" AND auditTrail."+key+" LIKE '%"+(String) filterMap.get(key)+"%' ");
				}
	...
	public static void auditLog(Document passport,String action,Document entity,String description,String approvedBy) {
		try {
			...
			if(passport!=null) {
				...
				at.setDoneBy("anonymous");
				at.setSessionId("noSession");
			}

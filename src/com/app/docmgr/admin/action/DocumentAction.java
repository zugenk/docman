    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (allowableAction.contains(actionRoot)) { 
	    	if("list".equalsIgnoreCase(action)){
    			...
    			if (UserService.getInstance().hasPrivilege(loginUser,"ADMIN:DOCUMENT_ACTIVATE")) forward = doActivateConfirm(mapping, form, request, response);
	    		else 	forward = mapping.findForward("not_authorized");
	    	}else if("activate_ok".equalsIgnoreCase(action)){
	    		if (UserService.getInstance().hasPrivilege(loginUser,"ADMIN:DOCUMENT_ACTIVATE")) doActivateOk(mapping, form, request, response);
	    		else 	forward = mapping.findForward("not_authorized");
    			...
    			if (UserService.getInstance().hasPrivilege(loginUser,"ADMIN:DOCUMENT_ARCHIVE")) forward = doArchiveConfirm(mapping, form, request, response);
	    		else 	forward = mapping.findForward("not_authorized");
	    	}else if("archive_ok".equalsIgnoreCase(action)){
	    		if (UserService.getInstance().hasPrivilege(loginUser,"ADMIN:DOCUMENT_ARCHIVE")) doArchiveOk(mapping, form, request, response);
	    		else 	forward = mapping.findForward("not_authorized");
    			...
    			if (UserService.getInstance().hasPrivilege(loginUser,"ADMIN:DOCUMENT_BLOCK")) forward = doBlockConfirm(mapping, form, request, response);
	    		else 	forward = mapping.findForward("not_authorized");
	    	}else if("block_ok".equalsIgnoreCase(action)){
	    		if (UserService.getInstance().hasPrivilege(loginUser,"ADMIN:DOCUMENT_BLOCK")) doBlockOk(mapping, form, request, response);
	    		else 	forward = mapping.findForward("not_authorized");

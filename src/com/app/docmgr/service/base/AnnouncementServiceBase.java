...
public class AnnouncementServiceBase {
	private static Logger log = Logger.getLogger("com.app.docmgr.service.base.AnnouncementServiceBase");	
	// private static AnnouncementServiceBase instance = null;
	 ...
	 */
	 /*
	public static synchronized AnnouncementServiceBase getInstance(){
		if(instance == null){
			instance = new AnnouncementServiceBase();
		}
		return instance;
	}
	*/
	 ...
	 * Service Class is dedicated to handle all data access to database
	 */
	public Announcement get(Long id) throws Exception{
		Announcement announcement = null;
		Session session = ConnectionFactory.getInstance().getSession();
		try {
			announcement = (Announcement) session.get(Announcement.class, id);
			Hibernate.initialize(announcement.getAnnouncementType());			
			Hibernate.initialize(announcement.getStatus());			
	 ...
	 */
	public Announcement getBy(String filterParam) throws Exception{
		Announcement announcement = null;
		Session session = ConnectionFactory.getInstance().getSession();
		try {
			String filter = " WHERE announcement.status.state='active'  ";
			if(filterParam!=null) filter = filter + filterParam;					
			Query query = session.createQuery("SELECT announcement FROM com.app.docmgr.model.Announcement announcement "+filter+" ");
			announcement = (com.app.docmgr.model.Announcement) query.uniqueResult();
	...
	public void add(Announcement announcement, Session session) throws Exception {
		Transaction trx = null;
		try	{
			trx = session.beginTransaction();
			session.save(announcement);
	...
	public void update(Announcement announcement, Session session) throws Exception {
		Transaction trx = null;
		try	{
			trx = session.beginTransaction();
			session.saveOrUpdate(announcement);
	...
	public void delete(Announcement announcement, Session session) throws Exception {
		Transaction trx = null;
		try	{
			trx = session.beginTransaction();
			session.delete(announcement);
	...
	public PartialList getPartialList(String filterParam, String orderParam, int start, int count) throws Exception{
		...
		try {
			...
			Query queryCount = session.createQuery("SELECT count(*) FROM com.app.docmgr.model.Announcement announcement "+filter+" ");
			...
			while(itr.hasNext()){
				com.app.docmgr.model.Announcement announcement = (com.app.docmgr.model.Announcement)itr.next();
				Hibernate.initialize(announcement.getAnnouncementType());			
				Hibernate.initialize(announcement.getStatus());			
			}			
		} catch(HibernateException he) {
	...
	public List getList(String filterParam, String orderParam) throws Exception{
		if(filterParam!=null) filterParam = " and announcement.status.state='active' "+filterParam;
		else filterParam=" and announcement.status.state='active' ";

...
public class DocumentServiceBase {
	private static Logger log = Logger.getLogger("com.app.docmgr.service.base.DocumentServiceBase");	
	// private static DocumentServiceBase instance = null;
	 ...
	 */
	 /*
	public static synchronized DocumentServiceBase getInstance(){
		if(instance == null){
			instance = new DocumentServiceBase();
		}
		return instance;
	}
	*/
	 ...
	 */
	public Document getBy(String filterParam) throws Exception{
		Document document = null;
		Session session = ConnectionFactory.getInstance().getSession();
		try {
	...
	public void add(Document document, Session session) throws Exception {
		Transaction trx = null;
		try	{
			trx = session.beginTransaction();
			session.save(document);
	...
	public void update(Document document, Session session) throws Exception {
		Transaction trx = null;
		try	{
			trx = session.beginTransaction();
			session.saveOrUpdate(document);
	...
	public void delete(Document document, Session session) throws Exception {
		Transaction trx = null;
		try	{
			trx = session.beginTransaction();
			session.delete(document);
	...
	public List getList(String filterParam, String orderParam) throws Exception{
		if(filterParam!=null) filterParam = " and document.status.state='active' "+filterParam;
		else filterParam=" and document.status.state='active' ";

 ...
 */
    ...
    public void doActivateOk(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response){
       	try{
       		Lookup lookup = (Lookup) request.getSession().getAttribute("lookup");
    		if(lookup == null){
    			response.sendRedirect("lookup.do?action=activate_confirm");
    		}
    		lookup.setStatus(StatusService.getInstance().getByTypeandCode("Lookup","activated"));
    		LookupService.getInstance().update(lookup);
    		response.sendRedirect("lookup.do?action=detail");    		
    	}catch(Exception ex){
    		try{
    ...
    public void doArchiveOk(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response){
       	try{
       		Lookup lookup = (Lookup) request.getSession().getAttribute("lookup");
    		if(lookup == null){
    			response.sendRedirect("lookup.do?action=archive_confirm");
    		}
    		lookup.setStatus(StatusService.getInstance().getByTypeandCode("Lookup","archived"));
    		LookupService.getInstance().update(lookup);
    		response.sendRedirect("lookup.do?action=detail");    		
    	}catch(Exception ex){
    		try{
    ...
    public void doBlockOk(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response){
       	try{
       		Lookup lookup = (Lookup) request.getSession().getAttribute("lookup");
    		if(lookup == null){
    			response.sendRedirect("lookup.do?action=block_confirm");
    		}
    		lookup.setStatus(StatusService.getInstance().getByTypeandCode("Lookup","blocked"));
    		LookupService.getInstance().update(lookup);
    		response.sendRedirect("lookup.do?action=detail");    		
    	}catch(Exception ex){
    		try{

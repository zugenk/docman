...
public class DocumentHistoryActionBase extends Action{
    ...
    public ActionForward doList(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response){
		...
		String param_documentHistory_parentFolder_filter = "";
		if(request.getParameter("documentHistory_parentFolder_filter")!=null){
			param_documentHistory_parentFolder_filter = request.getParameter("documentHistory_parentFolder_filter");
			if(param_documentHistory_parentFolder_filter.length() > 0 ){				
				documentHistory_filterSb.append("  AND documentHistory.parentFolder = '"+param_documentHistory_parentFolder_filter+"' ");
			}
		}		
		request.getSession().setAttribute("documentHistory_parentFolder_filter", param_documentHistory_parentFolder_filter);
		String param_documentHistory_parentDocument_filter = "";
		if(request.getParameter("documentHistory_parentDocument_filter")!=null){
			param_documentHistory_parentDocument_filter = request.getParameter("documentHistory_parentDocument_filter");
			if(param_documentHistory_parentDocument_filter.length() > 0 ){				
				documentHistory_filterSb.append("  AND documentHistory.parentDocument = '"+param_documentHistory_parentDocument_filter+"' ");
			}
		}		
		request.getSession().setAttribute("documentHistory_parentDocument_filter", param_documentHistory_parentDocument_filter);
...
*/ 			com.app.docmgr.model.Folder  parentFolderObj =null;
			...
			try{
				...
				if(parentFolderStr == null || parentFolderStr.trim().length() == 0 ){
					documentHistory.setParentFolder(null);
				}else{			
					parentFolderObj = parentFolderService.get(new Long(parentFolderStr));
					documentHistory.setParentFolder(parentFolderObj);
				}
			}catch(Exception ex){}	
			if(parentFolderObj==null){
				errors.add("documentHistory.parentFolder", new ActionError("error.documentHistory.parentFolder"));
			}
			...
			try{
				...
				if(parentDocumentStr == null || parentDocumentStr.trim().length() == 0 ){
					documentHistory.setParentDocument(null);
				}else{			
					parentDocumentObj = parentDocumentService.get(new Long(parentDocumentStr));
					documentHistory.setParentDocument(parentDocumentObj);
				}
			}catch(Exception ex){}	

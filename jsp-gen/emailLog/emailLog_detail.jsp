...
<form name="emailLog" action="emailLog.do" method="POST">
	...
	<TABLE border="0" width="98%" align="center" cellpadding="0" cellspacing="0">
		...
		<tr>
			...
			<td>
				<% 
				if(com.app.docmgr.action.EmailLogAction.allowableAction.contains("edit")) { 
						if (privilegeList.contains("EMAIL_LOG_EDIT")) { %>

					<input type="submit" value="<bean:message key="button.edit"/>" onclick="this.form.action.value='edit'" />
					&nbsp;
				<% 		}
					} %>
				<% if(com.app.docmgr.action.EmailLogAction.allowableAction.contains("delete")) { 
						if (privilegeList.contains("EMAIL_LOG_DELETE")) { %>
					<input type="button" value="<bean:message key="button.delete"/>" onclick="this.form.action.value='delete_confirm';this.form.submit()" />
					&nbsp;
				<% 		}
					} %>
				<% if(com.app.docmgr.action.EmailLogAction.allowableAction.contains("submit")) { 
						if (privilegeList.contains("EMAIL_LOG_SUBMIT")) { %>
				<input type="button" value="<bean:message key="button.submit"/>" onclick="this.form.action.value='submit_confirm';this.form.submit()" />
				&nbsp;
				<% 		}
					} %>
				<% if(com.app.docmgr.action.EmailLogAction.allowableAction.contains("approve")) { 
						if (privilegeList.contains("EMAIL_LOG_APPROVE")) { %>
				<input type="button" value="<bean:message key="button.approve"/>" onclick="this.form.action.value='approve_confirm';this.form.submit()" />
				&nbsp;
				<% 		}
					} %>
				<% if(com.app.docmgr.action.EmailLogAction.allowableAction.contains("reject")) { 
						if (privilegeList.contains("EMAIL_LOG_REJECT")) { %>
				<input type="button" value="<bean:message key="button.reject"/>" onclick="this.form.action.value='reject_confirm';this.form.submit()" />
				&nbsp;
				<% 		}
					} %>
				<% if(com.app.docmgr.action.EmailLogAction.allowableAction.contains("pending")) { 
						if (privilegeList.contains("EMAIL_LOG_PENDING")) { %>
				<input type="button" value="<bean:message key="button.pending"/>" onclick="this.form.action.value='pending_confirm';this.form.submit()" />
				&nbsp;
				<% 		}
					} %>
				<% if(com.app.docmgr.action.EmailLogAction.allowableAction.contains("process")) { 
						if (privilegeList.contains("EMAIL_LOG_PROCESS")) { %>
				...
				<% if(com.app.docmgr.action.EmailLogAction.allowableAction.contains("archive")) { 
						if (privilegeList.contains("EMAIL_LOG_ARCHIVE")) { %>
				...
				<% if(com.app.docmgr.action.EmailLogAction.allowableAction.contains("cancel")) { 
						if (privilegeList.contains("EMAIL_LOG_CANCEL")) { %>

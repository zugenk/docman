...
<form name="user" action="user.do" method="POST">
	...
	<TABLE border="0" width="98%" align="center" cellpadding="0" cellspacing="0">
		...
		<tr>
			...
			<td>
				<% 
				if(com.app.docmgr.action.UserAction.allowableAction.contains("edit")) { 
						if (privilegeList.contains("USER_EDIT")) { %>

					<input type="submit" value="<bean:message key="button.edit"/>" onclick="this.form.action.value='edit'" />
					&nbsp;
				<% 		}
					} %>
				<% if(com.app.docmgr.action.UserAction.allowableAction.contains("delete")) { 
						if (privilegeList.contains("USER_DELETE")) { %>
					<input type="button" value="<bean:message key="button.delete"/>" onclick="this.form.action.value='delete_confirm';this.form.submit()" />
					&nbsp;
				<% 		}
					} %>
				<% if(com.app.docmgr.action.UserAction.allowableAction.contains("submit")) { 
						if (privilegeList.contains("USER_SUBMIT")) { %>
				<input type="button" value="<bean:message key="button.submit"/>" onclick="this.form.action.value='submit_confirm';this.form.submit()" />
				&nbsp;
				<% 		}
					} %>
				<% if(com.app.docmgr.action.UserAction.allowableAction.contains("approve")) { 
						if (privilegeList.contains("USER_APPROVE")) { %>
				<input type="button" value="<bean:message key="button.approve"/>" onclick="this.form.action.value='approve_confirm';this.form.submit()" />
				&nbsp;
				<% 		}
					} %>
				<% if(com.app.docmgr.action.UserAction.allowableAction.contains("reject")) { 
						if (privilegeList.contains("USER_REJECT")) { %>
				<input type="button" value="<bean:message key="button.reject"/>" onclick="this.form.action.value='reject_confirm';this.form.submit()" />
				&nbsp;
				<% 		}
					} %>
				<% if(com.app.docmgr.action.UserAction.allowableAction.contains("pending")) { 
						if (privilegeList.contains("USER_PENDING")) { %>
				<input type="button" value="<bean:message key="button.pending"/>" onclick="this.form.action.value='pending_confirm';this.form.submit()" />
				&nbsp;
				<% 		}
					} %>
				<% if(com.app.docmgr.action.UserAction.allowableAction.contains("process")) { 
						if (privilegeList.contains("USER_PROCESS")) { %>
				<input type="button" value="<bean:message key="button.process"/>" onclick="this.form.action.value='process_confirm';this.form.submit()" />
				&nbsp;
				<% 		}
					} %>
				<% if(com.app.docmgr.action.UserAction.allowableAction.contains("activate")) { 
						if (privilegeList.contains("USER_ACTIVATE")) { %>
				...
				<% if(com.app.docmgr.action.UserAction.allowableAction.contains("remove")) { 
						if (privilegeList.contains("USER_REMOVE")) { %>
				<input type="button" value="<bean:message key="button.remove"/>" onclick="this.form.action.value='remove_confirm';this.form.submit()" />
				&nbsp;
				<% 		}
					} %>
				<% if(com.app.docmgr.action.UserAction.allowableAction.contains("block")) { 
						if (privilegeList.contains("USER_BLOCK")) { %>
				<input type="button" value="<bean:message key="button.block"/>" onclick="this.form.action.value='block_confirm';this.form.submit()" />
				&nbsp;
				<% 		}
					} %>
				<% if(com.app.docmgr.action.UserAction.allowableAction.contains("cancel")) { 
						if (privilegeList.contains("USER_CANCEL")) { %>
